...
export default function StatisticsBoxes({ baseColor, stats }) {
  ...
  const colors = [
    ...
    `rgb(${intToValidColorInt(base.r + 15)},${intToValidColorInt(base.g + 15)},${intToValidColorInt(base.b + 15)})`,
    `rgb(${intToValidColorInt(base.r - 15)},${intToValidColorInt(base.g - 15)},${intToValidColorInt(base.b - 15)})`,
  ]

  while (colors[1] === `` || theme.palette.getContrastText(colors[1]) !== contrastColor) {
    colors[1] = `rgb(${getRandomColorInt(base.r)},${getRandomColorInt(base.g)},${getRandomColorInt(base.b)})`
  }

  while (colors[2] === `` || theme.palette.getContrastText(colors[2]) !== contrastColor) {
    const c1 = xolor(colors[1])

    colors[2] = `rgb(${getRandomColorInt(c1.r)},${getRandomColorInt(c1.g)},${getRandomColorInt(c1.b)})`
  }

  return (
    <section className={classes.root}>
      {stats.map((stat, i) => (
        <div className={classes.stat} key={stat.caption} style={{ background: colors[i] }}>
          <H2
            style={{
